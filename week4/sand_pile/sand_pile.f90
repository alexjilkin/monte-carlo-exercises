program sand_pile
  ...
  call open_file(pile_unit,"sand_pile.dat")
  call open_file(stat_unit,"statistics.dat")
...
contains
  ...
  subroutine write_pile(unit,h,L)
    ...
    do i=1,L
      ...
      if (ios/=0) then
        write(0,"(a)") "*** Error writing sand pile to file"
        stop
      endif
    end do
  ...
  subroutine write_stat_header(unit)
    ...
    if (ios/=0) then
      write(0,"(a)") "*** Error writing statistics header to file"
      stop
    endif
  ...
  subroutine write_statistics(unit,grain,ntopple,hsum,hmax)
    ...
    if (ios/=0) then
      write(0,"(a)") "*** Error writing statistics to file"
      stop
    endif
